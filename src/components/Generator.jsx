  ...
  return (
    ...
    >
        ...
        {Object.keys(WORKOUTS).map((type, typeIndex) => {
          return (
            <button
              ...
              className={
                "capitalize duration-200 cursor-pointer p-4 grid bg-slate-950 font-light rounded-lg border hover:border-blue-600 " +
                (type === poison ? "border-blue-800 " : "border-transparent")
              }
              key={typeIndex}
            >
      ...
      <div className="bg-slate-950 flex flex-col  border border-solid m-3  border-blue-400 rounded-lg ">
        <button
          onClick={toggleModel}
          className="relative cursor-pointer p-3 flex items-center justify-center"
        >
          ...
          <FaCaretDown
            className={
              "size-6 absolute right-3 top-1/2 -translate-y-1/2 transition-transform duration-300 " +
      ...
      {error && <p className="text-sm text-center p-3 text-red-600">{error}</p>}
